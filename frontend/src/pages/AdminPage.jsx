...
export default function AdminPage({onLogout}) {
  ...
  const handleUpdateUser = async () => {
    if (!editUser.password || !editUser.group) {
      setError('All fields required');
      return;
    }
    setError('');
            ...
            <div style={{
              ...
              <div style={{
                ...
                <input placeholder="Username" value={newUser.username}
                       onChange={e => setNewUser(u => ({...u, username: e.target.value}))} style={{flex: 1}}/>
                <input placeholder="Password" value={newUser.password}
                       onChange={e => setNewUser(u => ({...u, password: e.target.value}))} style={{flex: 1}}/>
                <input placeholder="Email" value={newUser.email}
                       onChange={e => setNewUser(u => ({...u, email: e.target.value}))} style={{flex: 1}}/>
                  ...
                  {filteredUsers.map(u => (
                    <tr key={u.id} style={{background: editUser && editUser.id === u.id ? '#f0f8ff' : undefined}}>
                      ...
                      <td>{editUser && editUser.id === u.id ? (
                        <select value={editUser.group}
                                onChange={e => setEditUser(eu => ({...eu, group: e.target.value}))}>
                          <option value="untrained">untrained</option>
                          <option value="exposure">exposure</option>
                          <option value="expert">expert</option>
                          <option value="admin">admin</option>
                      ...
                      <td>{editUser && editUser.id === u.id ? (
                        <input placeholder="Email" value={editUser.email || ''}
                               onChange={e => setEditUser(eu => ({...eu, email: e.target.value}))}/>
                      ) : u.email}</td>
                      ...
                      <td>
                        {editUser && editUser.id === u.id ? (
                          <>
                            ...
                            <button className="apple-btn" style={{background: '#eee', color: '#333'}}
                                    onClick={() => setEditUser(null)}>Cancel
                            </button>
                          </>
                        ) : (
                          <>
                            <button className="apple-btn" onClick={() => handleEditUser({...u, password: ''})}>Edit
                            </button>
                            <button className="apple-btn" style={{background: '#fff0f0', color: '#e00'}}
                                    onClick={() => handleDeleteUser(u.id)}>Delete
                            </button>
                          </>
                        )}
                      </td>
